...
export const signup = async (
  ...
  try {
    console.log("🚀 Signup payload sending:", { email, username, password, confirmPassword });

    const res = await api.post(`${AUTH_BASE}register/`, {
      email,
      username,
      password,
      confirm_password: confirmPassword, // ✅ backend expects this name
    });
    ...
    if (err instanceof Error) {
      // Generic JS error
      console.error("❌ Signup failed (generic error):", err.message);
    } else if (typeof err === "object" && err && "response" in err) {
      // Axios-style error
      const axiosErr = err as any;
      console.error("❌ Signup failed (Axios error):", axiosErr.response?.data);
    } else {
      console.error("❌ Signup failed (unknown error):", err);
    }

    throw err; // Re-throw so AuthContext or SignupForm can handle it
  }
};
